"""
glpharm 전용 Ecount 런타임 코어.
변경 이유: glpharm 판매현황/구매현황 실행 진입을 공통 런타임과 분리해 운영 실수를 줄입니다.
"""
...
class GlpharmEcountCrawler:
    """glpharm 기업 코드로만 동작하는 얇은 래퍼."""

    def __init__(self) -> None:
        self._crawler = EcountCrawler(company_code=GLPHARM_COMPANY_CODE)
    ...
    async def crawl_purchase(
        ...
        """
        변경 이유: glpharm 구매현황 실행 시 page_type과 메뉴를 고정해 호출 실수를 방지합니다.
        """
        ...
        return {
            ...
            "rows": [],
            "inserted": 0,
            "error": "glpharm 구매현황 결과가 비어 있습니다.",
    ...
    async def crawl_sales(
        ...
        """
        변경 이유: glpharm 판매현황 실행 시 page_type과 메뉴를 고정해 호출 실수를 방지합니다.
        """
        results = await self._crawler.crawl_multi_menus_and_save(
        ...
        return {
            ...
            "rows": [],
            "inserted": 0,
            "error": "glpharm 판매현황 결과가 비어 있습니다.",
            "page_type": "search_excel",
        }
