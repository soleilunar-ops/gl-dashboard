...
from ecount_glpharm_runtime_core import default_date_to
from ecount_steps.glpharm_purchase_crawler import GlpharmPurchaseCrawler
from ecount_steps.glpharm_sales_crawler import GlpharmSalesCrawler
...
async def run_pipeline(
    ...
    """
    변경 이유: glpharm 판매/구매 전용 멀티 파이프라인 엔트리를 분리해 운영 시 혼선을 줄입니다.
    """
    purchase = GlpharmPurchaseCrawler()
    sales = GlpharmSalesCrawler()
    ...
    for label, runner in steps:
        ...
        print(
            f"[EcountGlpharmPipeline] 단계 완료: {label} | "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="glpharm Ecount 멀티 파이프라인 (구매현황 -> 판매현황)")
...
async def _main() -> None:
    ...
    print("\n" + "#" * 60)
    print("[EcountGlpharmPipeline] 결과 요약")
    print("#" * 60)
    for row in results:
        print(
